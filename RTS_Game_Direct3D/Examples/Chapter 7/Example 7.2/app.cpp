//////////////////////////////////////////////////////////////
// Example 7.2: Animating the Bones							//
...
HRESULT APPLICATION::Init(HINSTANCE hInstance, int width, int height, bool windowed)
{
	...
	RegisterClass(&wc);
	m_mainWindow = CreateWindow("D3DWND", "Example 7.2: Animating the Bones", WS_EX_TOPMOST, 0, 0, width, height, 0, 0, hInstance, 0); 
	SetCursor(NULL);
	...
	m_activeAnimation = m_animations.size() - 3;
...
{	
	...
	{
		m_activeAnimation++;
		if(m_activeAnimation >= m_animations.size())
			m_activeAnimation = 0;
...
HRESULT APPLICATION::Render()
{
    ...
    if(SUCCEEDED(m_pDevice->BeginScene()))
    {
		...
		m_pFont->DrawText(NULL, str.c_str(), -1, &r[1], DT_LEFT| DT_TOP | DT_NOCLIP, 0xff000000);
