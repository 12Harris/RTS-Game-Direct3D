//////////////////////////////////////////////////////////////
// Example 4.4: Creating the Terrain Mesh					//
...
HRESULT APPLICATION::Init(HINSTANCE hInstance, int width, int height, bool windowed)
{
	...
	RegisterClass(&wc);
	m_mainWindow = CreateWindow("D3DWND", "Example 4.4: Creating the Terrain Mesh", WS_EX_TOPMOST, 0, 0, width, height, 0, 0, hInstance, 0); 
	SetCursor(NULL);
...
HRESULT APPLICATION::Update(float deltaTime)
{
	...
	else if(KEYDOWN('F'))
	{
		...
		m_image++;
		if(m_image > 1)m_image = 0;

		if(m_terrain.m_pHeightMap != NULL)
		{
			m_terrain.m_pHeightMap->m_maxHeight = 10.0f;
			if(m_image == 0)m_terrain.m_pHeightMap->LoadFromFile(m_pDevice, "images/abe.jpg");
			if(m_image == 1)m_terrain.m_pHeightMap->LoadFromFile(m_pDevice, "images/smiley.bmp");
