//////////////////////////////////////////////////////////////
// Example 4.11: Placing Terrain Objects					//
...
HRESULT APPLICATION::Init(HINSTANCE hInstance, int width, int height, bool windowed)
{
	...
	RegisterClass(&wc);
	m_mainWindow = CreateWindow("D3DWND", "Example 4.11: Placing Terrain Objects", WS_EX_TOPMOST, 0, 0, width, height, 0, 0, hInstance, 0); 
	SetCursor(NULL);
...
HRESULT APPLICATION::Update(float deltaTime)
{
	...
	D3DXVECTOR2 centre = D3DXVECTOR2(m_terrain.m_size.x / 2.0f, m_terrain.m_size.y / 2.0f);
...
HRESULT APPLICATION::Render()
{
    ...
    if(SUCCEEDED(m_pDevice->BeginScene()))
    {
		...
		char number[50];
		std::string text = itoa(m_terrain.m_objects.size(), number, 10);
		text += " objects";
		RECT rc = {710, 10, 0, 0};
