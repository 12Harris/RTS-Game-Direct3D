...
void TERRAIN::Init(IDirect3DDevice9* Dev, INTPOINT _size)
{
	...
	m_objectPS.Init(Dev, "shaders/objects.ps", PIXEL_SHADER);
	m_objectVS.Init(Dev, "shaders/objects.vs", VERTEX_SHADER);
	m_objMatW = m_objectVS.GetConstant("matW");
	m_objMatVP = m_objectVS.GetConstant("matVP");
	m_objDirToSun = m_objectVS.GetConstant("DirToSun");
	m_objMapSize = m_objectVS.GetConstant("mapSize");
...
void TERRAIN::Render(CAMERA &camera)
{
	...
	//Render Objects
	m_objectVS.SetMatrix(m_objMatW, world);
	m_objectVS.SetMatrix(m_objMatVP, vp);
	m_objectVS.SetVector3(m_objDirToSun, m_dirToSun);
	m_objectVS.SetVector3(m_objMapSize, D3DXVECTOR3(m_size.x, m_size.y, 0.0f));
	...
	for(int i=0;i<m_objects.size();i++)
		if(!camera.Cull(m_objects[i].m_BBox))
		{
			...
			m_objectVS.SetMatrix(m_objMatW, m);
			m_objects[i].Render();
		}
