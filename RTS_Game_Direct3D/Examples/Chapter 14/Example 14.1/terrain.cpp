...
void TERRAIN::SetOrthogonalView()
{
	...
	//Projection matrix
	D3DXMatrixOrthoLH(&proj, m_size.x-1, m_size.y-1, 0.1f, 2000.0f);
...
{
	memset(m_pVisibleTiles, true, sizeof(bool) * m_size.x * m_size.y);
	memset(m_pVisitedTiles, true, sizeof(bool) * m_size.x * m_size.y);
	...
	for(int i=0;i<mapObjects.size();i++)
		...
		{
			int sr = mapObjects[i]->sightRadius, a = 0;
...
void TERRAIN::RenderLandscape()
{
	...
	//D3DXSaveTextureToFile("landScape.bmp", D3DXIFF_BMP, m_pLandScape, NULL);
}
