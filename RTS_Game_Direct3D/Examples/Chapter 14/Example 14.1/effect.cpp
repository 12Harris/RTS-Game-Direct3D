...
IDirect3DTexture9* selectedTexture = NULL;
IDirect3DTexture9* trainingTexture = NULL;
...
void LoadEffectResources(IDirect3DDevice9 *m_pDevice)
{
	...
	D3DXCreateTextureFromFile(m_pDevice, "textures/selected.dds", &selectedTexture);
	D3DXCreateTextureFromFile(m_pDevice, "textures/training.dds", &trainingTexture);
}
...
void UnloadEffectResources()
{
	...
	if(trainingTexture)trainingTexture->Release();
...
{
	m_t1.m_pos.y += 0.1f;
	m_color = D3DXVECTOR4(0.2f, 0.3f, 1.0f, 0.0f);

	for(int i=0;i<10;i++)
	{
		float angle = i * (D3DX_PI / 5.0f);
			
		m_c[i].Init(_pos + D3DXVECTOR3(cos(angle) * 0.1f, 0.7f, sin(angle) * 0.1f),
		          D3DXVECTOR3(D3DX_PI * 0.5f, angle, 0.0f),
				  D3DXVECTOR3(1.5f, 1.5f, 2.5f));
	}
}
...
void EFFECT_SPELL::Update(float timeDelta)
{
	...
	if(m_time < 1.5f)m_t1.m_sca += D3DXVECTOR3(timeDelta, timeDelta, timeDelta) * 1.2f;
	if(m_time > 4.5f)m_t1.m_sca -= D3DXVECTOR3(timeDelta, timeDelta, timeDelta) * 1.2f;
...
EFFECT_FIREBALL::EFFECT_FIREBALL(IDirect3DDevice9 *Dev, D3DXVECTOR3 *bonePos, MAPOBJECT *_target, MAPOBJECT *_attacker) : EFFECT(Dev)
{
	m_pSrcBone = bonePos;
	m_pTarget = _target;
	m_pAttacker = _attacker;
	dest = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_color.w = 0.01f;
	m_prc = 0.0f;
	m_speed = 3.0f;
	m_damage = m_pAttacker->m_damage;

	if(m_pTarget != NULL)
		dest = m_pTarget->m_position + D3DXVECTOR3(0.0f, 1.0f, 0.0f);

	if(m_pSrcBone != NULL)
		m_t1.Init(*m_pSrcBone, D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(1.0f, 1.0f, 1.0f));
}
...
void EFFECT_FIREBALL::Update(float timeDelta)
{
	try
	{
		...
		{
			dest = m_pTarget->m_position + D3DXVECTOR3(0.0f, 1.0f, 0.0f);
			if(m_pTarget->m_dead)m_color.w -= timeDelta * 0.5f;
		}

		if(m_time < 1.0f && m_pSrcBone != NULL)		//Follow staff
		{
			m_time += timeDelta;
			m_t1.m_pos = *m_pSrcBone;		//Extract position from bone
			m_t1.m_sca = D3DXVECTOR3(0.3f, 0.3f, 0.3f) * m_time;
			m_color.w = m_time;
		...
		else if(m_prc < 1.0f)		//Fly towards target
		{
			...
			if(m_prc >= 1.0f && m_pTarget != NULL)
				m_pTarget->Damage(m_damage, m_pAttacker);
		}
		else					//Explode
		{
			m_prc += (m_speed * timeDelta) / m_length;
			m_t1.m_sca += D3DXVECTOR3(1.0f, 1.0f, 1.0f) * timeDelta;
